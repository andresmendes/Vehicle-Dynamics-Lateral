%% Estudo comparação - DELTA = 45 graus
% Este script mostra o desempenho dos veículos para um ângulo de
% esterçamento de 45 graus e condições iniciais nulas.
%
%% Objetivo
...
% Dados
veiculoDados = 3; % Não ocorre variação dos dados do veículo
...
for i = 1:length(veiculoModeloVet)
	for j = 1:length(pneuModeloVet)
        ...
        % Modelo de veículo da iteração i
...
ylabel('dPSI [grau/s]')
xlabel('Tempo [s]')
l = legend(pneuModeloTxt(1,:),pneuModeloTxt(2,:),pneuModeloTxt(3,:),'Location','East');

if i == 3
    set(l,'Location','South');
end
...
title(strcat('Âng. de deriva - Esterçamento: 45 graus - Veículo: ',veiculoModeloTxt(i,:)));
...
ylabel('Distância [m]')
xlabel('Distância [m]')
legend(pneuModeloTxt(1,:),pneuModeloTxt(2,:),pneuModeloTxt(3,:),'Location','North')
