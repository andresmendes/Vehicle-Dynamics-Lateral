...
% dianteiro é nulo e as condições iniciais nulas iguais a dPSI = 2 [rad/s]
% e vy = 0 [m/s]]
%
...
for i = 1:length(veiculoModeloVet)
	for j = 1:length(pneuModeloVet)
		...
		% 
		T = 1.5; % Tempo total de simulação
...
if j==2
	...
	tamanho = 5;
		...
		set(H1,'Marker',pneuModeloMarcador(j),'MarkerFaceColor',pneuModeloCor(j),'MarkerSize',tamanho)
		ax = get(H1,'Parent');
%		set(ax,'XLim',[-0.3 0.05])
		title('PLANO DE FASE')
		xlabel('alphat [rad]')
		ylabel('dpsi [rad/s]')
		if j == length(pneuModeloVet)
		leg = legend(pneuModeloTxt(1,:),pneuModeloTxt(2,:),pneuModeloTxt(3,:),'Location','East');
		set(leg,'FontSize',7,'Box','on')
		end
		...
		box on
		grid on
		H2 = plot(TOUT,ALPHAR*180/pi,pneuModeloCor(j));
		set(H2,'Marker',pneuModeloMarcador(j),'MarkerFaceColor',pneuModeloCor(j),'MarkerSize',tamanho)
	...
	ind = int2str(i);
	%print(fig,strcat('resultados/comparacaoDpsi2/comparacaoDpsi2_',ind,'.eps'),'-depsc2')
end
